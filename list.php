...
$hookmanager->initHooks(array('stackbuilderconnectorlist'));
...
llxHeader('', $langs->trans('StackBuilderConnectorList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->stackbuilderconnector->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'stackbuilderconnector t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'stackbuilderconnector_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('StackBuilderConnector', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_stackbuilderconnector', 'GET');
...
$r = new Listview($db, 'stackbuilderconnector');
echo $r->render($sql, array(
    ...
    ,'list' => array(
        'title' => $langs->trans('StackBuilderConnectorList')
        ,'image' => 'title_generic.png'
        ,'picto_precedent' => '<'
        ,'picto_suivant' => '>'
        ,'noheader' => 0
        ,'messageNothing' => $langs->trans('NoStackBuilderConnector')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => StackBuilderConnector::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
...
function _getObjectNomUrl($id, $ref)
{
	...
	$o = new StackBuilderConnector($db);
