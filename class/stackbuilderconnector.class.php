...
class StackBuilderConnector extends SeedObject
{
	...
	public static $TStatus = array(
		self::STATUS_CANCELED => 'StackBuilderConnectorStatusShortCanceled'
		,self::STATUS_DRAFT => 'StackBuilderConnectorStatusShortDraft'
		,self::STATUS_VALIDATED => 'StackBuilderConnectorStatusShortValidated'
//		,self::STATUS_REFUSED => 'StackBuilderConnectorStatusShortRefused'
//		,self::STATUS_ACCEPTED => 'StackBuilderConnectorStatusShortAccepted'
	);

	/** @var string $table_element Table name in SQL */
	public $table_element = 'stackbuilderconnector';

	/** @var string $element Name of the element (tip for better integration in Dolibarr: this value should be the reflection of the class name with ucfirst() function) */
	public $element = 'stackbuilderconnector';
...
//        ),
    ...
    /**
     * StackBuilderConnector constructor.
    ...
    private function getNextRef()
    {
		...
		$mask = !empty($conf->global->STACKBUILDERCONNECTOR_REF_MASK) ? $conf->global->STACKBUILDERCONNECTOR_REF_MASK : 'MM{yy}{mm}-{0000}';
		$ref = get_next_value($db, $mask, 'stackbuilderconnector', 'ref');
...
//            $this->fk_user_valid = $user->id;
    ...
    public function getNomUrl($withpicto = 0, $moreparams = '')
    {
        ...
        $result='';
        $label = '<u>' . $langs->trans("ShowStackBuilderConnector") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;

        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/stackbuilderconnector/card.php', 1).'?id='.$this->id.urlencode($moreparams).$linkclose;
...
//        $picto='stackbuilderconnector@stackbuilderconnector';
    ...
    public static function LibStatut($status, $mode)
    {
        ...
        if ($status==self::STATUS_CANCELED) { $statusType='status9'; $statusLabel=$langs->trans('StackBuilderConnectorStatusCancel'); $statusLabelShort=$langs->trans('StackBuilderConnectorStatusShortCancel'); }
        elseif ($status==self::STATUS_DRAFT) { $statusType='status0'; $statusLabel=$langs->trans('StackBuilderConnectorStatusDraft'); $statusLabelShort=$langs->trans('StackBuilderConnectorStatusShortDraft'); }
        elseif ($status==self::STATUS_VALIDATED) { $statusType='status1'; $statusLabel=$langs->trans('StackBuilderConnectorStatusValidated'); $statusLabelShort=$langs->trans('StackBuilderConnectorStatusShortValidate'); }
        elseif ($status==self::STATUS_REFUSED) { $statusType='status5'; $statusLabel=$langs->trans('StackBuilderConnectorStatusRefused'); $statusLabelShort=$langs->trans('StackBuilderConnectorStatusShortRefused'); }
        elseif ($status==self::STATUS_ACCEPTED) { $statusType='status6'; $statusLabel=$langs->trans('StackBuilderConnectorStatusAccepted'); $statusLabelShort=$langs->trans('StackBuilderConnectorStatusShortAccepted'); }
